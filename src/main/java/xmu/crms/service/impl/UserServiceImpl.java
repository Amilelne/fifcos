...
public class UserServiceImpl implements UserService {
    ...
    public BigInteger insertAttendanceById(BigInteger classId, BigInteger seminarId, BigInteger userId, double longitude, double latitude) throws LocationNotFoundException, InvalidOperationException {
        ...
        if(location.getStatus().equals(Location.CALLING)){
            // 正常签到
            int insert=userMapper.insertAttendanceById(classId, seminarId, userId, Attendance.PRESENT);
            List<Attendance> attendances=userMapper.listAttendanceById(classId, seminarId);
            BigInteger attendanceId=new BigInteger("0");
            for(Attendance attendance:attendances){
                if(attendance.getStudent().getId()==userId){
            ...
            // 迟到
            int insert=userMapper.insertAttendanceById(classId, seminarId, userId, Attendance.LATE);
            List<Attendance> attendances=userMapper.listAttendanceById(classId, seminarId);
            BigInteger attendanceId=new BigInteger("0");
            for(Attendance attendance:attendances){
                if(attendance.getStudent().getId()==userId){
            ...
            // 缺勤
            int insert=userMapper.insertAttendanceById(classId, seminarId, userId, Attendance.ABSENCE);
            List<Attendance> attendances=userMapper.listAttendanceById(classId, seminarId);
            BigInteger attendanceId=new BigInteger("0");
            for(Attendance attendance:attendances){
                if(attendance.getStudent().getId()==userId){
    ...
    @Override
    public List<String> listAbsenceStudent(BigInteger seminarId, BigInteger classId) throws LocationNotFoundException{
        List<String> users = userMapper.listUnCallingStudent(seminarId, classId);
        if(users == null || users.isEmpty()){
