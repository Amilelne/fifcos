...
//            }
    ...
    @PreAuthorize("hasRole('TEACHER')")
    @RequestMapping(value = "/{seminarId}/class/{classId}/attendance/end",method = RequestMethod.GET)
    public ResponseEntity endCallInRoll(@PathVariable BigInteger seminarId,
                                           @PathVariable BigInteger classId){
        try{
    ...
    public ResponseEntity getAbsentStudentList(@PathVariable int seminarId, @PathVariable int classId){
        List<String> listAbsent = new ArrayList<String>();
        try {
            List<String> listTotal = userService.listAbsenceStudent(BigInteger.valueOf(seminarId), BigInteger.valueOf(classId));
            for (String aListTotal : listTotal) {
                //UserVO userVO = new UserVO(aListTotal.getId(), aListTotal.getName());
                listAbsent.add(aListTotal);
            }
        } catch (LocationNotFoundException e) {
            e.printStackTrace();
        }
